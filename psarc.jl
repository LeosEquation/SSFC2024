...
using .ImplicitFunction, TaylorSeries, LinearAlgebra


function first_step(f::Function,x_ini::Float64,p_ini::Float64,p_fin::Float64,orden::Int64)
    t = Taylor1(orden)

    x_s = 1/sqrt(1/(Implicit_function(f,x_ini,p_ini,orden,1)^2) + 1)
    p_s = sign(p_fin - p_ini)/sqrt(Implicit_function(f,x_ini,p_ini,orden,1)^2 + 1)
