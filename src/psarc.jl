...
function first_step(f::Function,x_ini::Float64,p_ini::Float64,p_fin::Float64,orden::Int64)
    ...
    x_s = 1/sqrt(1/(Implicit_function(f,x_ini,p_ini,orden)^2) + 1)
    p_s = sign(p_fin - p_ini)/sqrt(Implicit_function(f,x_ini,p_ini,orden)^2 + 1)
...
function first_step(f!::Function,x_ini::Vector{Float64},p_ini::Float64,p_fin::Float64,orden::Int64)
    ...
    p_s = sign(p_fin - p_ini)/sqrt(norm(Implicit_function(f!,x_ini,p_ini,orden))^2 + 1)
    x_s = abs(p_s)*Implicit_function(f!,x_ini,p_ini,orden)
    return x_s, p_s
...
function step(f!::Function,x_ini::Vector{Float64}, p_ini::Float64, x_s::Vector{Float64},p_s::Float64,orden::Int64)
    ...
    f_x = Jacobian(f!,x_ini,p_ini,orden)
    f_p = Jacobian(f!,x_ini,p_ini+Taylor1(orden))
...
function first_step(f!::Function,x_ini::Vector{Float64},p_ini::Vector{Float64},p_fin::Float64,orden::Int64, indice::Int64)
    p_s = sign(p_fin - p_ini[indice])/sqrt(norm(Implicit_function(f!,x_ini,p_ini,orden,indice))^2 + 1)
    x_s = abs(p_s)*Implicit_function(f!,x_ini,p_ini,orden,indice)
    return x_s, p_s
...
function step(f!::Function,x_ini::Vector{Float64}, p_ini::Vector{Float64}, x_s::Vector{Float64},p_s::Float64,orden::Int64,indice::Int64)
    ...
    f_x = Jacobian(f!,x_ini,p_ini,orden)
    f_p = Jacobian(f!,x_ini,p_ini,orden,indice)
